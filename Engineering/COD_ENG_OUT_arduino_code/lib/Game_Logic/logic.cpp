...
cla_session::cla_session(uint8_t _u8_player_quantity,
  ...
  for (int i = 0; i < _u8_player_quantity; i++) {
    ...
    array_players[i] =
      ...
      array_players[i] =
          new cla_computer_player(i, 5 + i * 10, _u8_computer_quantity,
                         this, 1); // ID = i, Startposition = 5 + i * 10
    }
  }
};
...
cla_computer_player::cla_computer_player(uint8_t _u8_player_id,
                                         ...
                                         cla_session *_obj_my_session,
                                         uint8_t _u8_en_mode) 
    : cla_player(_u8_player_id, _u8_start_position, _u8_computer_quantity, _obj_my_session) {
...
cla_manual_player::cla_manual_player(uint8_t _u8_player_id,
  ...
  uint8_t cla_player::Move_Token(uint8_t _u8_token_number,
    ...
    possible position is not occupied u8_token_position[_u8_token_number] =
    ...
    u8_is_occupied_token_number, _u8_affected_track_position) == u8_player_id &&
               u8_possible_position <
                   45) { // token is on track, position is occupied by other
    token
                         // of same player and would not move into the finish
                         // -> other token of the same player gets moved one
    field
                         // further
      Move_Token(u8_is_occupied_player_id, u8_is_occupied_token_number,
    _u8_affected_track_position, _u8_token_number, _u8_dice_value + 1); return
    0;
